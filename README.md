...
这是一个简单的网页应用，旨在提供免费的在线图片压缩服务。用户可以上传自己的图片，工具会对其进行优化，减小文件大小，并提供下载。

## 项目目标

为用户提供一个无需注册、简单易用的图片压缩工具，同时保证用户数据的隐私和安全。
...
- **`index.html` - 首页 / 图片压缩页**
    - **用途**: 这是应用的主页面，提供图片压缩的核心功能。
    - **布局**:
        - **头部 (Header)**: 包含网站 Logo 和导航栏，方便用户切换到其他功能页面。
        - **主内容区 (Main)**:
            - 一个醒目的标题，告诉用户这个页面的功能。
            - 一个图片上传区域，用户可以点击或拖拽图片到这里。
            - 上传后，会显示图片预览和压缩选项。
            - 处理完成后，会显示压缩后图片的信息和下载按钮。
        - **页脚 (Footer)**: 包含版权信息、隐私政策等链接。

- **`jpg-to-pdf.html` - JPG 转 PDF 工具页**
    - **用途**: 提供将 JPG 格式图片转换成 PDF 文件的功能。
    - **布局**: 结构与首页类似，主内容区提供 JPG 图片上传和转换为 PDF 的功能。

- **`text-remover.html` - 图片文字消除工具页**
    - **用途**: 提供一个工具，可以擦除或移除图片中的文字部分。
    - **布局**: 结构与首页类似，主内容区提供图片上传和文字擦除的功能。

- **`privacy-policy.html` - 隐私政策页**
    - **用途**: 详细说明我们如何处理用户上传的数据，保护用户隐私。
    - **布局**:
        - **头部 (Header)**: 包含网站 Logo 和返回主页的链接。
        - **主内容区 (Main)**: 包含详细的隐私政策条款。
        - **页脚 (Footer)**: 包含版权信息。

## 样式说明

- **`style.css` - 全局样式表**
    - **用途**: 定义整个网站的统一样式，包括颜色、字体、布局、按钮样式等。
    - **设计**: 采用简洁、现代的设计风格，确保在桌面和移动设备上都有良好的视觉效果和用户体验。

## 未来规划

1.  **完善核心功能**: 实现 `index.html` 的图片上传、压缩和下载逻辑。
2.  **开发其他工具**: 逐步实现 JPG 转 PDF 和图片文字消除的功能。
3.  **性能优化**: 对图片和代码进行优化，提升页面加载速度。
4.  **增加更多格式支持**: 除了 JPG、PNG，未来可能支持 GIF、WEBP 等更多图片格式的压缩。
